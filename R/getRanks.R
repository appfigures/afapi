...
#'
#' \code{getRanks} requests rank data for specific products from the
#' appFigures web API. 
#'
#' @param product_ids Numeric. A vector of appFigures-assigned
#' product ids.
#' 
#' @param country Character. One or more country iso abbreviations that
#' filter which countries rank data to include. Defaults to "US".
#' 
...
#' records that are ranked 1 - 100. Defaults to 400.
#' 
#' @param granularity Character. Should ranks be reported on a daily or
#' hourly basis.
#' 
#' @param tz Character. Specifies which timezone to use. Defaults to
#' 'est'.
#' 
...
#' 
#' @details The \code{category} argument is not included in the
#' web request. Subsetting the returned rank data on this argument
#' occurs after the data has been transformed into R objects and is
#' included purely as a convenience. Details on store categories
#' can be requested using \code{\link{getStoreData}}.
#' 
#' For \code{start_date} and \code{end_date}, if the supplied argument
#' can be interepreted as a date or POSIX object, any hour, minute, or
#' second is ignored. If a string is supplied, it should have the
#' format: 'yyyy-MM-dd'.
#' 
...
#'
#' \code{parseRanks} parses the JSON returned by an app rank
...
#'
#' \code{getRankSnapshot} requests data for the top ranked apps for
#' for a specific hour from the appFigures web API. 
#'
#' @param tsmp Either the string "current" (default) or a string
#' with the format: "yyyy-MM-ddTHH". Only the last 24 hours are
#' available. 
#' 
#' @param country Chatacter. A country iso2 abbreviation. Defaults
#' to 'US'.
#' 
#' @param category Numeric. The numeric category of the requested
#' snapshot. Defaults to \code{25204} (top overall).
#' 
#' @param subcategory Character. The sub-category of the requested
#' snapshot. Defaults to \code{"free"}.
#' 
#' @param count Numeric. The total number of the ranks to return.
#' See Details.
#' 
...
#' without being converted to R objects? Defaults to \code{FALSE}.
#' 
#' @details The use of the \code{start} argument differs from the
#' official API documentation. A value such as \code{start = 15}
#' will return a list of products starting with a rank value of 15.
#' If \code{start = 15} and \code{count = 10}, then products data
#' for apps ranked from 15 - 24 will be returned.
#' 
...
#' Retrieve multiple rank snapshots.
#'
#' \code{stackSnapshots} requests data for the top ranked apps for
#' for multiple time stamps from the appFigures web API.
#' 
#' @param tsmps Either a character vector of elements with the
#' format: "yyyy-MM-ddTHH", or a vector of POSIX time stamps.
...
#' print to the console? Defaults to \code{FALSE}.
#' 
#' @param ... Other arguments to pass to \code{\link{getRankSnapshot}}.
#' Currently, \code{stackSnapshots} is simple a wrapper to
#' \code{getRankSnapshot}.
#' 
#' @details See \code{\link{getRankSnapshot}}.
#' 
#' @return A data frame containing product and category data for 
#' a cross section of top ranked apps.
#' 
...
stackSnapshots <- function(tsmps, curlHandle = NULL, verbose = F, ...) {
  #timestamps - a character vector of timestamps in POSIXt format;
  #             dates should be on the hour
  #
  tsmps <- as.character(tsmps)
  tsmps  <- paste(substr(tsmps, 1,10), "T", substr(tsmps, 12, 13), sep = "")
  if (missing(curlHandle)) {
    opts <- list(userpwd = paste(USERNAME, PASSWORD, sep = ":"),
                 httpheader = c('X-Client-Key' = API_KEY),
                 httpauth = 1L, verbose = verbose, ssl.verifypeer = FALSE)
