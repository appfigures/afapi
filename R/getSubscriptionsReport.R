#' Retrieve Subscription reports.
#'
#' \code{getSubscriptionReport} provides access to detailed subscription metrics.
#'
#' @param product_ids Numeric. A vector of appFigures-assigned
#' product ids. The default will return subscription reports for all products
#' in the account.
#' 
...
#' \code{"date", "product", "store", "country"}
#' 
...
#' 
#' @return A data frame containing the requested subscription report or the
#' report in json format, depending on the selected options.
#' 
#' @seealso Official documentation: 
#' \url{http://docs.appfigures.com/api/reference/v2/subscriptions}.
#' 

getSubscriptionsReport <- function(product_ids, end_date, start_date,
  group_by, country, format = c("flat", "csv", "json"),
  granularity = c("daily", "weekly", "monthly", "yearly"),
  curlHandle, verbose = FALSE, orgJSON = FALSE) {
  
  uri <- paste(BASE_URI, "reports", "subscriptions", sep = "/")
  ...
  parList <- c(products = product_ids, end_date = end_date,
               ...
               granularity = granularity, format = format,
               group_by = group_by)
  ...
  cont_vars <- c("trial_conversion_rate", "mrr", "actual_revenue",
                 "churn", "gross_revenue", "gross_mrr")
...
getAdSpendReport <- function(product_ids, end_date, start_date,
  ...
  uri <- paste(BASE_URI, "reports", "adspend", sep = "/")
  granularity = match.arg(granularity)
  format <- match.arg(format)
  if (missing(end_date)) {
    end_date <- as.character(Sys.Date())
...
getRevenueReport <- function(product_ids, end_date, start_date,
  ...
  uri <- paste(BASE_URI, "reports", "revenue", sep = "/")
  granularity = match.arg(granularity)
  format <- match.arg(format)
  if (missing(end_date)) {
    end_date <- as.character(Sys.Date())
  ...
  parList <- c(products = product_ids, end_date = end_date,
               start_date = start_date, countries = country,
               granularity = granularity, format = format, group_by = group_by)
  if (missing(curlHandle)) {
    opts <- list(userpwd = paste(USERNAME, PASSWORD, sep = ":"),
                 httpheader = c('X-Client-Key' = API_KEY),
                 httpauth = 1L, verbose = verbose, ssl.verifypeer = FALSE)
  ...
  cont_vars <- c("sales", "iap", "returns", "edu", "ads", "total", "gross_sales",
                 "gross_iap", "gross_edu", "gross_returns", "gross_total")
  for (cv in intersect(cont_vars, names(output))) {
    output[, cv] <- as.numeric(output[, cv])
  }
  if (all(c("start_date", "end_date") %in% names(output))) {
