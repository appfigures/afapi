...
parseProducts <- function(jsonText) {
  ...
  if (length(datr[[1]]) == 1) {
    out <- data.frame(
      ...
      release_date = as.POSIXct(datr[["release_date"]], format = "%Y-%m-%dT%H:%M:%S"),
      added_date = as.POSIXct(datr[["added_date"]], format = "%Y-%m-%dT%H:%M:%S"),
      updated_date = as.POSIXct(datr[["updated_date"]], format = "%Y-%m-%dT%H:%M:%S"),
    ...
    out <- data.frame(
      ...
      release_date = as.POSIXct(vapply2("release_date", "character"), format = "%Y-%m-%dT%H:%M:%S"),
      added_date = as.POSIXct(vapply2("added_date", "character"), format = "%Y-%m-%dT%H:%M:%S"),
      updated_date = as.POSIXct(vapply2("updated_date", "character"), format = "%Y-%m-%dT%H:%M:%S"),
